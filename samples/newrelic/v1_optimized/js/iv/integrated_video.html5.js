...
(function () {
    ...
    IV.Html5 = function(args) {
		var is_chrome = /chrome/ig.test(navigator.userAgent),
			...
			$video = $('#iv'),
			$poster = $('#iv-poster'),
			$posterbg = $('.iv-bg-poster'),
	        self = this,
            vID;
		
			$poster.css({'display':'block'});
			$posterbg.css({'display':'block'});
        ...
        var preload = function () {
				// if (is_chrome || is_safari){
					...
					if(!IV.isMobile()){
						setTimeout(function () {
							// $video.css({'opacity':1});
							$('#iv_html5_api').css({'opacity':1});
							console.log('this first');
							// $video.css({'display':'block'});
							setTimeout(function () {
								console.log('this second');
								$poster.css({'display':'none'});
								$posterbg.css({'display':'none'});
							},1500);
						},500);
					}
				// }	
				if (IV.isMobile()) {
					...
					$('#iv_html5_api').removeAttr('controls');
            ...
            init = function() {
                ...
                //self.el = self.player.a; 		//videojs-4.2.2/video.js usee 'a' for 'el'
        ...
        $(this.el).addClass("iv video-js vjs-quicktime-skin");
		...
		if(is_chrome){
			...
			this.player.src("video/new_relic_enterprise.mp4?i="+timestamp);
		}
    ...
    IV.Element.prototype.activate = function () {
        ...
        if (!/\iv-content\b/.test(self.content.className)) {
            self.content.className += " iv-content";
        }
    ...
    IV.Element.prototype.addPlayBtn = function () {
        var $btn, $container, x, y, btnH, btnW;
        ...
        $btn = $("<span/>");
        $btn.addClass("iv-play-btn");

        $container.append($btn);
        
        setTimeout(function () {
            $container.addClass("iv-highlight");
        }, 3000);
    ...
    IV.Element.prototype.start = function (callback) {
		...
		function TVLink() {
			var	currentTime = self.video.time(),
				logoTime = self.video.duration() - 4;
        ...
        function showWhenReady() {
            if (self.video.time() > startPoint) {
				...
				setTimeout(function () {
					$(self.container).removeClass("iv-inactive-controls");
					// $container.addClass("flash-controls");
				},2000);
        ...
        function hideWhenDone() {
			...
			$container.addClass("iv-inactive-controls");
		}
    ...
    IV.Element.prototype.show = function (callback) {
	   ...
	   // setTimeout(function(){
		$(self.container).addClass("iv-active");
	   // },2000);	
    ...
    IV.Element.prototype.hide = function (callback) {
        ...
        $container.removeClass("iv-active");
		$container.addClass('iv-inactive-controls');		
    };
